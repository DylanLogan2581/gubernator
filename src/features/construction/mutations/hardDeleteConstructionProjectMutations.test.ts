...
} from "./hardDeleteConstructionProjectMutations";
...
describe("hardDeleteConstructionProjectMutationOptions", () => {
  it("rejects invalid input before touching the Supabase client", async () => {
    ...
    const options = hardDeleteConstructionProjectMutationOptions({
      client,
      queryClient,
    ...
    ).rejects.toSatisfy(isHardDeleteConstructionProjectMutationError);
    expect(rpc).not.toHaveBeenCalled();
  });

  it("calls hard_delete_construction_project RPC and returns expected result", async () => {
    ...
    expect(calls.rpc).toHaveBeenCalledWith("hard_delete_construction_project", {
  ...
  it("raises hard_delete_construction_project_not_found when RPC returns no row", async () => {
  ...
  it("maps 42501 to hard_delete_construction_project_not_authorized", async () => {
  ...
  it("maps P0002 to hard_delete_construction_project_not_found", async () => {
    const { client } = createRpcClient({
      data: null,
  ...
  it("maps P0001 to hard_delete_construction_project_not_cancelled", async () => {
    const { client } = createRpcClient({
      data: null,
