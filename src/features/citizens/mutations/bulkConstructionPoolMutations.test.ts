...
} from "./bulkConstructionPoolMutations";
...
describe("setBulkConstructionPoolMutationOptions", () => {
  it("rejects invalid input before touching the Supabase client", async () => {
    ...
    const options = setBulkConstructionPoolMutationOptions({
      client,
      queryClient,
      worldId: WORLD_ID,
    });
    ...
    await expect(result).rejects.toSatisfy(isBulkConstructionPoolMutationError);
  ...
  it("calls set_bulk_construction_pool RPC and returns the mapped result", async () => {
    const row: RpcRow = {
      after: 3,
    ...
    expect(rpc).toHaveBeenCalledWith("set_bulk_construction_pool", {
      p_settlement_id: SETTLEMENT_ID,
      p_target_count: 3,
    });
    expect(options.mutationKey).toEqual([
      "citizens",
      "set-bulk-construction-pool",
    ]);
  ...
  it("raises bulk_assignment_failed when the RPC returns no row", async () => {
    ...
    ).rejects.toBeInstanceOf(BulkConstructionPoolMutationError);
    await expect(
      executeMutation(queryClient, options, VALID_INPUT),
