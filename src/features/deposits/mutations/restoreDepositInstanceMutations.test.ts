...
} from "./restoreDepositInstanceMutations";
...
describe("restoreDepositInstanceMutationOptions", () => {
  it("rejects invalid input before touching the Supabase client", async () => {
    ...
    const options = restoreDepositInstanceMutationOptions({
      client,
      queryClient,
    });
    ...
    ).rejects.toSatisfy(isRestoreDepositInstanceMutationError);
    expect(rpc).not.toHaveBeenCalled();
  });

  it("calls restore_deposit_instance RPC and returns expected result", async () => {
    const row: RpcRow = {
    ...
    expect(calls.rpc).toHaveBeenCalledWith("restore_deposit_instance", {
      p_deposit_instance_id: DEPOSIT_INSTANCE_ID,
    });
    expect(options.mutationKey).toEqual([
      "deposits",
      "restore-deposit-instance",
    ]);
  ...
  it("raises restore_deposit_instance_not_found when RPC returns no row", async () => {
    ...
    ).rejects.toMatchObject({ code: "restore_deposit_instance_not_found" });
  });

  it("maps 42501 to restore_deposit_instance_not_authorized", async () => {
  ...
  it("maps P0002 to restore_deposit_instance_not_found", async () => {
    const { client } = createRpcClient({
      data: null,
  ...
  it("maps P0001 to restore_deposit_instance_not_removed", async () => {
    const { client } = createRpcClient({
      data: null,
      error: { code: "P0001", message: "deposit instance is not removed" },
    });
    const queryClient = createQueryClient();
    const options = restoreDepositInstanceMutationOptions({
      client,
