...
} from "./removeDepositInstanceMutations";
...
describe("removeDepositInstanceMutationOptions", () => {
  it("rejects invalid input before touching the Supabase client", async () => {
    ...
    const options = removeDepositInstanceMutationOptions({
      client,
      queryClient,
    });
    ...
    ).rejects.toSatisfy(isRemoveDepositInstanceMutationError);
    expect(rpc).not.toHaveBeenCalled();
  });

  it("calls remove_deposit_instance RPC and returns expected result", async () => {
    const row: RpcRow = {
    ...
    expect(calls.rpc).toHaveBeenCalledWith("remove_deposit_instance", {
      p_deposit_instance_id: DEPOSIT_INSTANCE_ID,
    });
    expect(options.mutationKey).toEqual([
      "deposits",
      "remove-deposit-instance",
    ]);
  ...
  it("raises remove_deposit_instance_not_found when RPC returns no row", async () => {
    ...
    ).rejects.toMatchObject({ code: "remove_deposit_instance_not_found" });
  });

  it("maps 42501 to remove_deposit_instance_not_authorized", async () => {
    ...
    await expect(
      executeMutation(queryClient, options, VALID_INPUT),
    ).rejects.toMatchObject({ code: "remove_deposit_instance_not_authorized" });
  });

  it("maps P0002 to remove_deposit_instance_not_found", async () => {
    const { client } = createRpcClient({
      data: null,
  ...
  it("maps P0001 with 'already removed' to remove_deposit_instance_already_removed", async () => {
    const { client } = createRpcClient({
      data: null,
      error: { code: "P0001", message: "instance is already removed" },
    });
    const queryClient = createQueryClient();
    const options = removeDepositInstanceMutationOptions({
      client,
  ...
  it("maps generic P0001 to remove_deposit_instance_blocked", async () => {
    const { client } = createRpcClient({
      data: null,
      error: { code: "P0001", message: "workers still assigned" },
    });
    const queryClient = createQueryClient();
    const options = removeDepositInstanceMutationOptions({
      client,
    ...
    await expect(
      executeMutation(queryClient, options, VALID_INPUT),
    ).rejects.toMatchObject({ code: "remove_deposit_instance_blocked" });
  });
});
