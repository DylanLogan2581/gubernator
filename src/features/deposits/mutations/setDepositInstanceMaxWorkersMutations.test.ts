import { QueryClient } from "@tanstack/react-query";
import { describe, expect, it, vi } from "vitest";

import type { GubernatorSupabaseClient } from "@/lib/supabase";

import { depositsQueryKeys } from "../queries/depositsQueryKeys";

import {
  isSetDepositInstanceMaxWorkersMutationError,
  setDepositInstanceMaxWorkersMutationOptions,
} from "./setDepositInstanceMaxWorkersMutations";

const SETTLEMENT_ID = "11111111-1111-1111-1111-111111111111";
const DEPOSIT_INSTANCE_ID = "33333333-3333-3333-3333-333333333333";

const VALID_INPUT = {
  depositInstanceId: DEPOSIT_INSTANCE_ID,
  maxWorkers: 5,
  removalStrategy: "npc_first" as const,
  settlementId: SETTLEMENT_ID,
};

type RpcRow = {
  readonly max_workers: number | null;
  readonly unassigned_citizen_ids: string[];
};

type SupabaseError = { readonly code?: string; readonly message: string };
type SupabaseResult<T> =
  | { readonly data: T; readonly error: null }
  | { readonly data: null; readonly error: SupabaseError | null };

function createRpcClient(result: SupabaseResult<RpcRow>): {
  readonly client: GubernatorSupabaseClient;
  readonly calls: { readonly rpc: ReturnType<typeof vi.fn> };
} {
  const maybeSingle = vi.fn().mockResolvedValue(result);
  const rpc = vi.fn(() => ({ maybeSingle }));
  return {
    client: { rpc } as unknown as GubernatorSupabaseClient,
    calls: { rpc },
  };
}

function createQueryClient(): QueryClient {
  return new QueryClient({
    defaultOptions: { mutations: { retry: false }, queries: { retry: false } },
  });
}

function executeMutation<TOptions extends { mutationFn?: unknown }>(
  queryClient: QueryClient,
  options: TOptions,
  variables: unknown,
): Promise<unknown> {
  return queryClient
    .getMutationCache()
    .build(queryClient, options as never)
    .execute(variables);
}

describe("setDepositInstanceMaxWorkersMutationOptions", () => {
  it("rejects invalid input before touching the Supabase client", async () => {
    const rpc = vi.fn();
    const client = { rpc } as unknown as GubernatorSupabaseClient;
    const queryClient = createQueryClient();
    const options = setDepositInstanceMaxWorkersMutationOptions({
      client,
      queryClient,
    });

    await expect(
      executeMutation(queryClient, options, {
        ...VALID_INPUT,
        depositInstanceId: "not-a-uuid",
      }),
    ).rejects.toSatisfy(isSetDepositInstanceMaxWorkersMutationError);
    expect(rpc).not.toHaveBeenCalled();
  });

  it("calls set_deposit_instance_max_workers RPC and returns expected result", async () => {
    const row: RpcRow = { max_workers: 5, unassigned_citizen_ids: [] };
    const { client, calls } = createRpcClient({ data: row, error: null });
    const queryClient = createQueryClient();
    const invalidateSpy = vi.spyOn(queryClient, "invalidateQueries");
    const options = setDepositInstanceMaxWorkersMutationOptions({
      client,
      queryClient,
    });

    const result = await executeMutation(queryClient, options, VALID_INPUT);

    expect(result).toEqual({ maxWorkers: 5, unassignedCitizenIds: [] });
    expect(calls.rpc).toHaveBeenCalledWith("set_deposit_instance_max_workers", {
      p_deposit_instance_id: DEPOSIT_INSTANCE_ID,
      p_max_workers: 5,
      p_removal_strategy: "npc_first",
    });
    expect(options.mutationKey).toEqual([
      "deposits",
      "set-deposit-instance-max-workers",
    ]);
    expect(invalidateSpy).toHaveBeenCalledWith(
      expect.objectContaining({
        queryKey: depositsQueryKeys.instancesBySettlement(SETTLEMENT_ID),
      }),
    );
  });

  it("returns unassigned citizen ids when workers are bumped out", async () => {
    const CITIZEN_A = "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa";
    const row: RpcRow = {
      max_workers: 2,
      unassigned_citizen_ids: [CITIZEN_A],
    };
    const { client } = createRpcClient({ data: row, error: null });
    const queryClient = createQueryClient();
    const options = setDepositInstanceMaxWorkersMutationOptions({
      client,
      queryClient,
    });

    const result = await executeMutation(queryClient, options, VALID_INPUT);

    expect(result).toMatchObject({ unassignedCitizenIds: [CITIZEN_A] });
  });

  it("returns null maxWorkers when cap is removed", async () => {
    const row: RpcRow = { max_workers: null, unassigned_citizen_ids: [] };
    const { client } = createRpcClient({ data: row, error: null });
    const queryClient = createQueryClient();
    const options = setDepositInstanceMaxWorkersMutationOptions({
      client,
      queryClient,
    });

    const result = await executeMutation(queryClient, options, {
      ...VALID_INPUT,
      maxWorkers: null,
      removalStrategy: null,
    });

    expect(result).toMatchObject({ maxWorkers: null });
  });

  it("raises set_max_workers_not_found when RPC returns no row", async () => {
    const { client } = createRpcClient({ data: null, error: null });
    const queryClient = createQueryClient();
    const options = setDepositInstanceMaxWorkersMutationOptions({
      client,
      queryClient,
    });

    await expect(
      executeMutation(queryClient, options, VALID_INPUT),
    ).rejects.toMatchObject({ code: "set_max_workers_not_found" });
  });

  it("maps 42501 to set_max_workers_not_authorized", async () => {
    const { client } = createRpcClient({
      data: null,
      error: { code: "42501", message: "permission denied" },
    });
    const queryClient = createQueryClient();
    const options = setDepositInstanceMaxWorkersMutationOptions({
      client,
      queryClient,
    });

    await expect(
      executeMutation(queryClient, options, VALID_INPUT),
    ).rejects.toMatchObject({ code: "set_max_workers_not_authorized" });
  });

  it("maps P0002 to set_max_workers_not_found", async () => {
    const { client } = createRpcClient({
      data: null,
      error: { code: "P0002", message: "no rows" },
    });
    const queryClient = createQueryClient();
    const options = setDepositInstanceMaxWorkersMutationOptions({
      client,
      queryClient,
    });

    await expect(
      executeMutation(queryClient, options, VALID_INPUT),
    ).rejects.toMatchObject({ code: "set_max_workers_not_found" });
  });

  it("maps P0001 to set_max_workers_strategy_required", async () => {
    const { client } = createRpcClient({
      data: null,
      error: { code: "P0001", message: "removal strategy required" },
    });
    const queryClient = createQueryClient();
    const options = setDepositInstanceMaxWorkersMutationOptions({
      client,
      queryClient,
    });

    await expect(
      executeMutation(queryClient, options, VALID_INPUT),
    ).rejects.toMatchObject({ code: "set_max_workers_strategy_required" });
  });
});
