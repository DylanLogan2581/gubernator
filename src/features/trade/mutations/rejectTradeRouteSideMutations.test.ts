...
} from "./rejectTradeRouteSideMutations";
...
describe("rejectTradeRouteSideMutationOptions", () => {
  it("rejects invalid input before touching the Supabase client", async () => {
    ...
    ).rejects.toSatisfy(isRejectTradeRouteSideMutationError);
    expect(rpc).not.toHaveBeenCalled();
  });

  it("calls reject_trade_route_side RPC and returns expected result", async () => {
    const row: RpcRow = {
    ...
    expect(calls.rpc).toHaveBeenCalledWith("reject_trade_route_side", {
      p_rejector_citizen_id: CITIZEN_ID,
      p_route_id: TRADE_ROUTE_ID,
      p_side: "destination",
    });
    expect(options.mutationKey).toEqual(["trade", "reject-trade-route-side"]);
  ...
  it("raises reject_trade_route_side_not_found when RPC returns no row", async () => {
    ...
    ).rejects.toMatchObject({ code: "reject_trade_route_side_not_found" });
  });

  it("maps 42501 to reject_trade_route_side_not_authorized", async () => {
    ...
    await expect(
      executeMutation(queryClient, options, VALID_INPUT),
    ).rejects.toMatchObject({ code: "reject_trade_route_side_not_authorized" });
  });

  it("maps P0002 to reject_trade_route_side_not_found", async () => {
    const { client } = createRpcClient({
      data: null,
  ...
  it("maps P0001 with 'side nation' to reject_trade_route_side_citizen_wrong_nation", async () => {
    const { client } = createRpcClient({
      data: null,
  ...
  it("maps P0001 with 'current status' to reject_trade_route_side_invalid_status", async () => {
    const { client } = createRpcClient({
      data: null,
      error: { code: "P0001", message: "cannot reject in current status" },
    });
    const queryClient = createQueryClient();
    const options = rejectTradeRouteSideMutationOptions({
      client,
    ...
    await expect(
      executeMutation(queryClient, options, VALID_INPUT),
    ).rejects.toMatchObject({ code: "reject_trade_route_side_invalid_status" });
  });

  it("maps generic P0001 to reject_trade_route_side_values_invalid", async () => {
    const { client } = createRpcClient({
      data: null,
    ...
    await expect(
      executeMutation(queryClient, options, VALID_INPUT),
    ).rejects.toMatchObject({ code: "reject_trade_route_side_values_invalid" });
  });
});
