...
function makeRpcMock(): ReturnType<typeof vi.fn> {
...
describe("CancelConfirmDialog", () => {
  ...
  it("happy path with no traders — shows plain success toast", async () => {
    const user = userEvent.setup();
    const { onClose } = renderDialog({ traderCount: 0 });
  ...
  it("happy path with traders — includes trader count in toast", async () => {
    const user = userEvent.setup();
    renderDialog({ traderCount: 3 });

    expect(screen.getByText(/3 traders/)).toBeDefined();

    await user.click(screen.getByRole("button", { name: "Cancel route" }));

    await waitFor(() => {
      expect(toastSuccess).toHaveBeenCalledWith(
        "Trade route cancelled. 3 traders were unassigned.",
        undefined,
      );
    });
  });

  it("singular trader count uses 'trader was'", async () => {
    const user = userEvent.setup();
    renderDialog({ traderCount: 1 });

    await user.click(screen.getByRole("button", { name: "Cancel route" }));

    await waitFor(() => {
      expect(toastSuccess).toHaveBeenCalledWith(
        "Trade route cancelled. 1 trader was unassigned.",
        undefined,
      );
    });
  });
  ...
  it("keep button calls onClose without submitting", async () => {
    const user = userEvent.setup();
