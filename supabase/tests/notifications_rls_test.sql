...
-- Seed the notifications directly as postgres (bypasses RLS and INSERT grant)
-- so the visibility assertions below have data to read. The legacy
-- advance_world_turn_if_current RPC was dropped in §C35 (Epic 6); notifications
-- are now generated by apply_turn_transition. For an RLS-only test the source
-- doesn't matter — only that the rows exist with the expected shape.
insert into
  public.notifications (
    recipient_user_id,
    world_id,
