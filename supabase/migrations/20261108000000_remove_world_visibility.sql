-- Migration: remove_world_visibility
-- Removes the public/private visibility concept from worlds. Access is now
-- limited to super admins, world admins of the world, and users with a
-- living player_character in the world (via has_world_access /
-- current_user_has_world_access, unchanged elsewhere).
-- ---------------------------------------------------------------------------
-- 1. Drop the public-world SELECT policy on worlds
-- ---------------------------------------------------------------------------
drop policy "worlds_select_public" on public.worlds;

-- ---------------------------------------------------------------------------
-- 2. Redefine has_world_access: drop the visibility = 'public' arm. The
--    world-admin and super-admin arms are unchanged, so every RLS policy and
--    RPC that reuses has_world_access (directly or via
--    current_user_has_world_access) loses the public path consistently.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3. create_world: drop the p_visibility parameter
-- ---------------------------------------------------------------------------
drop function public.create_world (text, text);

create function public.create_world (p_name text) returns setof public.worlds language plpgsql security definer
set
  search_path = '' as $$
begin
...
revoke all on function public.create_world (text)
from
  public;

grant
execute on function public.create_world (text) to authenticated;

-- ---------------------------------------------------------------------------
-- 4. import_world_from_template: drop the p_visibility parameter
-- ---------------------------------------------------------------------------
drop function public.import_world_from_template (text, text, jsonb);

create function public.import_world_from_template (p_name text, p_template jsonb default '{}'::jsonb) returns setof public.worlds language plpgsql security definer
set
  search_path = '' as $$
declare
  -- New world
...
begin
  ...
  loop
    ...
    insert into public.resources (
      world_id, name, slug, base_stockpile_cap, change_mode, change_amount, is_system_resource
    )
    values (
      v_world_id,
      v_item->>'name',
...
revoke all on function public.import_world_from_template (text, jsonb)
from
  public;

grant
execute on function public.import_world_from_template (text, jsonb) to authenticated;

-- ---------------------------------------------------------------------------
-- 5. Drop worlds.visibility: check constraint, then column
-- ---------------------------------------------------------------------------
alter table public.worlds
drop constraint worlds_visibility_check;
