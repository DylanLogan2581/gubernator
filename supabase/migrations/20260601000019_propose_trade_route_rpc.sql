-- Migration: propose_trade_route_rpc
-- SECURITY DEFINER RPC that creates a new trade route proposal between two settlements.
-- Authorised callers: any user who manages either endpoint's nation via
-- current_user_manages_nation (super admin, world admin, or Nation Manager PC).
--
-- Error contract:
...
-- If a side has no active managers the world admins receive the notification
...
add constraint notifications_notification_type_check check (
  notification_type in ('turn.completed', 'trade_proposal_received')
);
