-- Migration: apply_turn_transition_stockpile_revalidation
-- Defense-in-depth re-validation for issue #506 / review finding H9.
--
-- After the world lock is acquired, re-reads the live stockpile quantity for
-- each stockpileDeltas entry and compares it to the quantityBefore the engine
-- computed from. Any mismatch means another mutation committed between the
-- edge-function's state-load and this RPC call; we raise P0001 so the caller
-- can retry with fresh state rather than silently overwriting the in-flight
-- admin change.
-- ---------------------------------------------------------------------------
