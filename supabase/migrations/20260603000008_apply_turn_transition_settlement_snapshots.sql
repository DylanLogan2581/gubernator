-- Migration: apply_turn_transition_settlement_snapshots
-- §C34: Extends apply_turn_transition to bulk-insert the engine's
-- settlementSnapshots list into settlement_turn_snapshots.
--
-- Idempotency: a partial unique index on (turn_transition_id, settlement_id)
-- prevents duplicate rows on retry. The WHERE clause scopes dedup to
-- non-null transition_ids so baseline backfill rows (turn_transition_id IS NULL)
-- are unaffected.
-- ---------------------------------------------------------------------------
-- Idempotency index for §C34.
create unique index if not exists settlement_turn_snapshots_transition_settlement_unique on public.settlement_turn_snapshots (turn_transition_id, settlement_id)
where
  turn_transition_id is not null;
