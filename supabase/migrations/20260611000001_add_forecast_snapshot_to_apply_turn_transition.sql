-- Migration: add_forecast_snapshot_to_apply_turn_transition
-- Adds forecast snapshot parameter to apply_turn_transition RPC and stores
-- it in turn_transitions.forecast_snapshot_jsonb for forecast-vs-actual comparison.
--
-- The forecast is computed from a dry-run simulation before the actual transition
-- and stored so players can later see what was predicted vs. what actually happened.
-- ---------------------------------------------------------------------------
...
begin
  ...
  begin
    ...
    from public.internal_apply_turn_transition_construction_patches (p_payload);

    -- §C29b: buildingStateChanges
    v_building_state_change_count := public.internal_apply_turn_transition_deposit_managed_pop_patches (
      p_world_id, p_payload
    );

    -- §C30: tradeRouteOutcomes
    v_trade_route_outcome_count := public.internal_apply_turn_transition_trade_route_patches (
      p_payload
    );

    -- §C32: Citizens — births, deaths, partnerships
    select
      backfill_count,
      citizen_birth_count,
      citizen_death_count,
    ...
    from public.internal_apply_turn_transition_citizen_partnership_patches (
      p_world_id, p_expected_turn_number, p_payload
    );

    -- §C33: logEntries, notifications
    select
      log_entry_count,
      notification_count
    into
    ...
    from public.internal_apply_turn_transition_log_entries_and_notifications (
      v_transition.id, p_payload
    );

    -- §C34: settlement_snapshots
