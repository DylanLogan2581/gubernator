-- Migration: add_icon_color_to_config_entities
-- Issue #1244: icon chip colors are currently assigned by hashing the entity
-- UUID into one of 8 palette slots (hashToCategoricalSlot), which is stable
-- but not user-controllable. Adds a nullable `icon_color smallint` (one of
-- the 8 theme palette slots) to the five config entity tables so admins can
-- pin a color; null keeps the existing hash-based fallback (resolved
-- client-side by resolveIconTone). Not added to resource_categories, which
-- keep their own hex color.
alter table public.resources
add column icon_color smallint null;
...
alter table public.resources
add constraint resources_icon_color_range_check check (
  icon_color is null
  or icon_color between 1 and 8
);

alter table public.job_definitions
add constraint job_definitions_icon_color_range_check check (
  icon_color is null
  or icon_color between 1 and 8
);

alter table public.building_blueprints
add constraint building_blueprints_icon_color_range_check check (
  icon_color is null
  or icon_color between 1 and 8
);

alter table public.deposit_types
add constraint deposit_types_icon_color_range_check check (
  icon_color is null
  or icon_color between 1 and 8
);

alter table public.managed_population_types
add constraint managed_population_types_icon_color_range_check check (
  icon_color is null
  or icon_color between 1 and 8
);

-- ---------------------------------------------------------------------------
-- settlement_stockpiles_view / resources_directory_view: expose
-- resources.icon_color alongside the existing icon column (same
-- drop/recreate constraint noted in 20260820000000_add_icon_to_config_entities.sql
-- -- column list changes, no CREATE OR REPLACE VIEW column-list narrowing).
-- ---------------------------------------------------------------------------
...
drop view public.resources_directory_view;
...
-- ---------------------------------------------------------------------------
-- import_world_from_template: thread icon_color through the resources, jobs,
-- blueprints, deposit_types, and managed_population_types inserts. Everything
-- else is unchanged from 20261112000000_remove_resource_category_icon.sql.
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- -------------------------------------------------------------------------
  -- 10. Deposit types (+icon, icon_color)
