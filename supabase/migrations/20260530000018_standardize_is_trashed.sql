-- Migration: standardize_is_trashed
-- Renames the soft-delete column to `is_trashed` (DEFAULT FALSE) across all
-- five epic-4 entities, replacing:
--   resources.is_deleted        DEFAULT FALSE  (semantics unchanged)
--   job_definitions.is_active   DEFAULT TRUE   (semantics INVERTED)
--   building_blueprints.is_active DEFAULT TRUE (semantics INVERTED)
--   deposit_types.is_active     DEFAULT TRUE   (semantics INVERTED)
--   managed_population_types.is_active DEFAULT TRUE (semantics INVERTED)
--
-- Order of operations:
--   1. Drop BEFORE triggers (reference old column names; must precede rename)
--   2. Drop partial unique indexes (reference old column names)
--   3. Rename columns on all five tables
--   4. Fix DEFAULT to false for the four is_active tables
--   5. Invert existing values for the four is_active tables
--   6. Recreate validation functions (reference column names in function bodies)
--   7. Recreate trigger functions and reattach triggers
--   8. Recreate partial unique indexes with WHERE NOT is_trashed
--   9. Recreate all 15 trash RPCs with is_trashed
-- ---------------------------------------------------------------------------
-- ===========================================================================
-- 1. Drop BEFORE triggers
-- ===========================================================================
drop trigger if exists job_definitions_enforce_referential_integrity on public.job_definitions;

drop trigger if exists resources_enforce_referential_integrity on public.resources;
...
rename column is_deleted to is_trashed;

alter table public.job_definitions
rename column is_active to is_trashed;
...
-- ===========================================================================
-- 4. Fix DEFAULT for the four tables that had DEFAULT TRUE
-- ===========================================================================
alter table public.job_definitions
alter column is_trashed
set default false;
...
-- resources.is_trashed was already DEFAULT FALSE; no change needed.
-- ===========================================================================
-- 5. Invert existing values for the four is_active tables
--    (is_active = true  → is_trashed = false;
--     is_active = false → is_trashed = true)
-- ===========================================================================
update public.job_definitions
set
  is_trashed = not is_trashed;
...
-- ===========================================================================
-- 6. Recreate validation functions
-- ===========================================================================
