-- Migration: remove_resource_category_icon
-- Issue #1243: resource_categories.icon was only ever rendered inside the
-- Manage Categories list itself, never referenced elsewhere in the app.
-- Categories now carry name + color only. Drops the column, recreates the
-- dependent resources_directory_view without category_icon, and updates
-- import_world_from_template to stop writing it (a v2 template that still
-- includes "icon" on a resource_categories entry is simply ignored, so old
-- exports keep importing).
-- ---------------------------------------------------------------------------
-- 1. resources_directory_view depends on resource_categories.icon and must
--    be dropped before the column can go.
-- ---------------------------------------------------------------------------
drop view public.resources_directory_view;

alter table public.resource_categories
drop constraint resource_categories_icon_max_length_check,
drop column icon;
...
-- ---------------------------------------------------------------------------
-- 2. import_world_from_template -- drop the icon column from the resource
--    categories insert (step 4). Everything else is unchanged from
--    20261110000000_world_template_v2.sql.
-- ---------------------------------------------------------------------------
