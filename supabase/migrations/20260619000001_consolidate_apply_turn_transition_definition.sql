-- Migration: consolidate_apply_turn_transition_definition
-- Consolidates apply_turn_transition RPC and its helpers into a single,
-- canonical source. Prior to this migration, the function and helpers
-- were redefined ~18 times across:
--   - 20260603000001 (initial skeleton)
--   - 20260603000002–09, 20260603000011, 20260603000013 (incremental patches)
--   - 20260604000001, 20260604000003, 20260604000006 (more patches)
--   - 20260604000007 (refactor: extract helpers, add cross-world guard)
--   - 20260606000003 (restriction: service_role only, auth check removal)
--   - 20260615000001 (helper fix: surname support)
--
-- This migration assembles the final, authoritative definitions:
--   - create_citizen_internal (from 20260615000001)
--   - 8 internal_apply_turn_transition_* helpers (from 20260604000007, with
--     internal_apply_turn_transition_citizen_partnership_patches from 20260615000001)
--   - apply_turn_transition orchestrator (from 20260606000003 — NOT 20260618000000,
--     which has a bug in the settlement_snapshots call)
--   - Permissions (from 20260606000003)
--
-- Note: 20260618000000 attempted to add deposit/managed-pop drift validation
-- but introduced a bug: it tries to call settlement_snapshots with the wrong
-- signature (v_transition.to_turn_number instead of p_world_id, and expecting
-- a two-column return). This consolidation uses the correct version from
-- 20260606000003, effectively fixing that bug as a side effect.
--
-- Later migrations (20260606000003, 20260615000001, 20260618000000) are
-- superseded by this migration but remain in the history for auditing.
-- ---------------------------------------------------------------------------
-- ============================================================
-- create_citizen_internal (used by citizenship patches)
-- ============================================================
...
-- ============================================================
-- Internal helper functions (§C28–§C35)
-- ============================================================
...
-- ============================================================
-- Main orchestrator: apply_turn_transition
-- ============================================================
