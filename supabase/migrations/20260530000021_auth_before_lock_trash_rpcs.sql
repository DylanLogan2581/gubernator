-- Migration: auth_before_lock_trash_rpcs
-- Reorders all 15 trash RPCs so the authorization check (is_super_admin /
-- is_world_admin) fires BEFORE the SELECT … FOR UPDATE row lock.
--
-- Old ordering: NULL guard → FOR UPDATE → row-exists check → auth check → mutate
-- New ordering: NULL guard → auth check → FOR UPDATE → row-exists check → mutate
--
-- Behavioral note: an unauthorized caller passing a non-existent entity ID now
-- receives 42501 (insufficient_privilege) rather than P0002 (no_data_found),
-- because auth is evaluated before the row lookup.  Authorized callers see the
