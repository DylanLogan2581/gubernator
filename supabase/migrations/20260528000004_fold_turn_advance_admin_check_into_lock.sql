-- Migration: fold_turn_advance_admin_check_into_lock
-- Eliminates the TOCTOU race in advance_world_turn_if_current by folding the
-- admin authority check (owner / world_admins / super-admin) into the same
-- SELECT ... FOR UPDATE that acquires the row lock, so authorization and lock
-- acquisition are atomic.
...
begin
  ...
  -- Acquire the FOR UPDATE lock on the world row and check admin authority in
  -- the same query so authorization cannot be invalidated between the check and
  -- the lock.
  select
