-- Migration: generalize_resource_decay_to_change_mode
-- Issue #1165: generalizes the percent-only decay_rate column into a signed
-- change_amount + change_mode (percent|flat) pair, so a resource can grow or
-- decay by a flat quantity or a percentage each turn. Existing decay_rate
-- values (a percent loss) are losslessly backfilled to change_mode='percent',
-- change_amount = -decay_rate.
-- ---------------------------------------------------------------------------
alter table public.resources
add column change_mode text not null default 'percent' constraint resources_change_mode_check check (change_mode in ('percent', 'flat')),
add column change_amount numeric(10, 2) not null default 0;
...
-- A percent change can decay by at most 100% of the stockpile in a turn;
-- percent growth and flat growth/decay are otherwise unbounded (storage cap
-- and the zero floor are enforced by the simulation, not the schema).
alter table public.resources
add constraint resources_change_amount_percent_range_check check (
  change_mode <> 'percent'
  or change_amount >= -100
);

alter table public.resources
drop column decay_rate;

-- ---------------------------------------------------------------------------
-- import_world_from_template — redefine to write change_mode/change_amount
-- instead of decay_rate. Body copied from the latest definition
-- (20260729000000_import_world_from_template.sql) with only the resources
-- insert/upsert block changed.
-- ---------------------------------------------------------------------------
