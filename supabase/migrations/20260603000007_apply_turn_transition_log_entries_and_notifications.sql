-- Migration: apply_turn_transition_log_entries_and_notifications
-- §C33: Extends apply_turn_transition to bulk-insert turn_log_entries and
-- fan out simulation notifications to scoped recipients (§4.15).
--
-- Notification scope rules (§11.10):
--   settlement → settlement manager + nation manager + world admins
--   nation     → nation manager + world admins
--   world      → world admins only
-- "World admins" includes the world owner, explicit world_admins rows, and
-- super admins. Falls back to world admins when no active manager exists.
--
-- Idempotency: a partial unique index on
--   (generated_in_transition_id, recipient_user_id, notification_type,
--    coalesce(settlement_id, sentinel_uuid))
-- prevents duplicate rows on retry.
-- ---------------------------------------------------------------------------
-- §4.15 idempotency index.
-- coalesce(settlement_id, sentinel_uuid) collapses NULLs so world-scoped and
-- nation-scoped notifications (settlement_id IS NULL) also deduplicate correctly.
create unique index if not exists notifications_transition_dedup_idx on public.notifications (
