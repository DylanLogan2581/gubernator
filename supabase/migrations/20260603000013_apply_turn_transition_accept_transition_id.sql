-- Migration: apply_turn_transition_accept_transition_id
-- Issue #497 / review finding C2: the engine RNG seed must equal turn_transitions.id.
--
-- Changes from the previous definition (20260603000011):
--   1. Drops the old 3-argument function and re-creates it with a 4th parameter:
--      p_transition_id uuid — the UUID returned by start_turn_transition, which the
--      edge function already used as the engine RNG seed.
--   2. Replaces the INSERT + ON CONFLICT block with a simple SELECT lookup that verifies
--      the row exists, belongs to p_world_id, and has status = 'running'.
--   3. Moves the §C36 cross-world payload guard before the failure-capture block so that
--      payload-validation failures leave the transition in 'running' status (the caller can
--      fix the payload and retry with the same transition id). Only mutations inside the
--      failure block can produce a 'failed' transition.
-- ---------------------------------------------------------------------------
-- Drop old signature before creating the new one (same name, different arity).
revoke
execute on function public.apply_turn_transition (uuid, integer, jsonb)
from
  public;

drop function public.apply_turn_transition (uuid, integer, jsonb);
...
begin
  ...
  -- start_turn_transition() is responsible for inserting this row before the engine runs.
  select
  ...
  -- §C36: Cross-world payload guard (moved before the failure-capture block).
  -- Payload-validation failures leave the transition in 'running' status so the
  -- caller can fix the payload and retry with the same p_transition_id.
  -- Build id-sets for each mutable entity type scoped to p_world_id, then walk
  -- every payload key and reject any ID that is not in the matching set.
