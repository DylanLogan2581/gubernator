-- Migration: events_management_rpcs
-- Implement RPC for atomic event group + events creation with multi-target support.
-- Also implement cancel RPC for event cancellation.
-- Input validation constraints matching src/lib/inputLimits.ts
-- eventGroupNameMax: 128, eventGroupDescriptionMax: 1000
-- ============================================================================
-- RPC: create_event_group_with_events
-- ============================================================================
-- Creates one event_group row and one event row per target atomically.
-- Validates world admin permission before proceeding.
--
-- Parameters:
--   p_world_id: UUID of the world
--   p_group_name: narrative name for the group
--   p_group_description: optional description
--   p_effect_type: e.g., 'plague', 'abundance', etc.
--   p_scope_type: 'world' | 'nation' | 'settlement'
--   p_targets: JSON array of target objects
--     Format: [
--       {
--         "scope_id": "uuid or null",
--         "nation_id": "uuid or null",
--         "settlement_id": "uuid or null",
--         "job_id": "bigint or null",
--         "building_blueprint_id": "uuid or null",
...
-- ============================================================================
-- RPC: cancel_event_or_group
-- ============================================================================
-- Cancels an event or all events in a group atomically.
--
-- Parameters:
--   p_event_id: UUID of event to cancel (if provided, cancels only that event)
--   p_group_id: UUID of group to cancel (if provided, cancels all events in group)
--   Either p_event_id or p_group_id must be provided (not both null)
--
-- Returns: JSON with count of cancelled events
...
begin
  ...
  -- Permission check
  if not (is_world_admin(v_world_id) or is_super_admin()) then
...
-- ============================================================================
-- RPC: update_event_group_with_events
-- ============================================================================
-- Updates an event group and its associated events atomically.
-- Scope and targets cannot be changed (locked for v1).
--
-- Parameters:
--   p_group_id: UUID of the event group to update
--   p_group_name: new narrative name for the group
--   p_group_description: new optional description
--   p_effects: JSON array of effect objects (replaces all existing effects)
--   p_duration_type: 'instant' | 'sustained'
--   p_duration_transitions: integer (required if sustained, ignored if instant)
--   p_activate_on_transition_after_turn_number: integer
--   p_create_citizen_memories: boolean
--   p_memory_text: text or null
--
-- Returns: JSON with updated group_id
