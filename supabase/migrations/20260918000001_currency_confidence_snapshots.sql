-- Migration: currency_confidence_snapshots
-- #1094: currency simulation persistence — nation_currency_snapshots (chart
-- data, one row per nation currency per turn), persisting the engine's
-- confidence/default updates back onto nation_currencies, and the two
-- currency notification types (currency.default, currency.confidence_collapsing).
--
-- The engine's nationalEconomy phase (phaseNationalEconomy.ts) now also
-- returns nationCurrencySnapshots / nationCurrencyUpdates, and emits
-- notifications entries with notificationType 'currency.default' /
-- 'currency.confidence_collapsing' (scope 'nation').
--
-- Architecture note: as of 20260629000002_generate_notifications_from_log_entries
-- (consumed fully into 20260915000001), apply_turn_transition's generic
-- notifications-array loop was removed — every other notification type is now
-- generated from turn_log_entries.log_category by
-- internal_apply_turn_transition_log_entries_and_notifications. The engine
-- does not (yet) emit a currency_default / currency_confidence_collapsing
-- log_category, only notifications-array entries, so this migration follows
-- the internal_apply_turn_transition_nation_economy precedent (20260907000000):
-- a standalone helper, called as its own step from apply_turn_transition, that
-- owns its own persistence — including reading the two currency notification
-- types directly out of p_payload -> 'notifications' (filtered to just those
-- two types, so nothing already handled by the giant log-category-driven
-- function is double-inserted here).
-- ---------------------------------------------------------------------------
-- nation_currency_snapshots
-- ---------------------------------------------------------------------------
create table public.nation_currency_snapshots (
  ...
  constraint nation_currency_snapshots_transition_world_fkey foreign key (turn_transition_id, world_id) references public.turn_transitions (id, world_id) on delete cascade,
  constraint nation_currency_snapshots_unique unique (turn_transition_id, currency_id)
);

create index nation_currency_snapshots_world_transition_idx on public.nation_currency_snapshots (world_id, turn_transition_id);

create index nation_currency_snapshots_nation_turn_idx on public.nation_currency_snapshots (nation_id, turn_number desc);

create index nation_currency_snapshots_currency_turn_idx on public.nation_currency_snapshots (currency_id, turn_number desc);

alter table public.nation_currency_snapshots enable row level security;

-- SELECT: any user with world access may read nation currency snapshots,
-- matching nation_turn_snapshots visibility (v1: world members).
create policy "nation_currency_snapshots_select_world_access" on public.nation_currency_snapshots for
select
  to authenticated using (public.current_user_has_world_access (world_id));

grant
select
  on public.nation_currency_snapshots to authenticated;

-- No INSERT/UPDATE/DELETE policies for authenticated: rows are written only
-- by the apply_turn_transition RPC below (SECURITY DEFINER, bypasses RLS/
-- grants as the function owner), matching nation_turn_snapshots' write
-- posture (20260907000000).
-- ---------------------------------------------------------------------------
-- internal_apply_turn_transition_nation_currency (§C38)
-- Persists one nation_currency_snapshots row per entry in
-- nationCurrencySnapshots, updates nation_currencies.confidence/
-- is_in_default from nationCurrencyUpdates, and inserts the two
-- currency-scoped notification types straight from the payload's
-- notifications array (nation scope; not handled by the log-category-driven
-- giant function since the engine does not emit a matching log_category for
-- these yet).
-- ---------------------------------------------------------------------------
...
begin
  ...
  loop
    ...
    values
      (
        ...
        coalesce((v_snapshot ->> 'confidence')::numeric, 1),
  ...
  loop
    ...
    set
      confidence = coalesce((v_update ->> 'confidence')::numeric, confidence)
    where
...
-- ---------------------------------------------------------------------------
-- Redefine apply_turn_transition with §C38 wired in. Body copied verbatim
-- from the latest definition (20260915000001, including its treaty-patches
-- pieces) with: cross-world guard loops for
-- nationCurrencySnapshots/nationCurrencyUpdates, the §C38 call site, and its
-- returned counts added to patchCounts.
-- ---------------------------------------------------------------------------
