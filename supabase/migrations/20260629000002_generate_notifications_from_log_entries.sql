-- Migration: generate_notifications_from_log_entries
-- Updates internal_apply_turn_transition_log_entries_and_notifications to generate
-- notifications via set-based inserts per notification type (instead of row-by-row
-- INSERT in a loop with UNION per notification).
--
-- Each notification type has optimized recipient scoping:
...
--
-- This migration replaces the loop-based approach with set-based INSERTs, improving
-- efficiency and enabling per-type recipient customization.
-- ---------------------------------------------------------------------------
