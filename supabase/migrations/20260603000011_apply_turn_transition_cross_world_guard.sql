-- Migration: apply_turn_transition_cross_world_guard
-- §C36: Adds an up-front payload-validation pass to apply_turn_transition.
--
-- Builds id-sets per entity table filtered by world_id = p_world_id before
-- any mutation and rejects any payload ID that does not belong to that world
-- with P0001 'cross-world id <uuid> in <payload key>'.
--
-- Affected payload keys (per issue #496 review finding C1):
--   stockpileDeltas, constructionUpdates, buildingsCreated,
--   buildingStateChanges, depositUpdates, managedPopulationUpdates,
--   tradeRouteOutcomes, bornOnTurnBackfill, citizenBirths,
--   citizenDeaths, partnershipChanges, assignmentClears
-- ---------------------------------------------------------------------------
...
begin
  ...
  begin
    -- §C36: Cross-world payload guard.
    -- Build id-sets for each mutable entity type scoped to p_world_id, then
    -- walk every payload key and reject any ID that is not in the matching set.
    -- This prevents a world admin of World A from mutating entities in World B
    -- by crafting a payload with cross-world IDs.
    ...
    -- Trade route: both endpoints must be in-world.
    ...
    -- §C32 pre-pass (§4.9): Apply born_on_turn_number backfill for existing citizens.
    -- Each entry: citizenId, bornOnTurnNumber.
    for v_backfill in
      select value
      from jsonb_array_elements(coalesce(p_payload -> 'bornOnTurnBackfill', '[]'::jsonb))
    loop
    ...
    -- Each entry: settlementId, name, sex, bornOnTurnNumber, parentACitizenId (nullable),
    -- parentBCitizenId (nullable), plus optional NPC flavor fields.
    -- Delegates to create_citizen_internal which enforces world and kinship invariants.
    -- Newborns have no assignment row (= unassigned by absence).
    for v_birth in
      select value
      from jsonb_array_elements(coalesce(p_payload -> 'citizenBirths', '[]'::jsonb))
    loop
    ...
    -- Each entry: citizenId, deathCauseCategory, deathCause.
    ...
    -- Each entry: citizenAId, citizenBId, toStatus ('active' | 'widowed' | 'dissolved'),
    -- formedOnTurnNumber (required for new formations), endedOnTurnNumber (for endings).
    -- 'active' inserts a new partnership row; 'widowed'/'dissolved' updates the
    -- existing active partnership between the two citizens.
    for v_partnership_change in
      select value
      from jsonb_array_elements(coalesce(p_payload -> 'partnershipChanges', '[]'::jsonb))
    loop
    ...
    -- Each entry: citizenId. Deletes the citizen_assignments row so the citizen
    -- becomes unassigned (used when a citizen dies or is otherwise released).
    for v_assignment_clear in
      select value
      from jsonb_array_elements(coalesce(p_payload -> 'assignmentClears', '[]'::jsonb))
    loop
    ...
    -- §C32e (decision §11.6): Stamp manual_deconstruct_overshoot log entries that
    -- were written out-of-band (turn_transition_id IS NULL) so that the homelessness
    -- pass (phase 10) can correlate them with this transition.
    ...
    -- Each entry carries: category, settlementId (optional), nationId (optional),
    -- citizenId (optional), resourceId (optional), payload (object), phase (string).
    -- The phase field is informational-only and not stored in turn_log_entries.
    ...
    -- Each entry carries: notificationType, messageText, scope
    -- ('settlement' | 'nation' | 'world'), settlementId (optional), nationId (optional).
    --
    -- Recipient resolution per §4.15:
    --   settlement scope → settlement manager + nation manager of that settlement's
    --                       nation + world admins (owner, world_admins, super admins)
    --   nation scope     → nation manager + world admins
    --   world scope      → world admins only
    -- "World admins" always included as the unconditional floor / fallback.
    --
    -- The partial unique index notifications_transition_dedup_idx prevents duplicates
    -- on retry; the ON CONFLICT clause relies on it.
    ...
    loop
      ...
      -- Derive nation_id from settlement when not explicitly supplied.
      ...
      select
        ...
        v_transition.id
      from (
        -- Settlement manager: alive PC with settlement_manager role for this settlement.
        select c.user_id
        from public.citizens c
        ...
        where v_notif_scope = 'settlement'
          ...
          and u.status = 'active'
        union
        -- Nation manager: alive PC with nation_manager role for the notification's nation.
        select c.user_id
        from public.citizens c
        inner join public.users u on u.id = c.user_id
    ...
    -- Each entry carries camelCase fields from the engine's SettlementSnapshot type:
    -- settlementId, turnNumber, aliveTotal, aliveNpc, alivePc, populationCap,
    -- birthCount, deathCount, starvationDeathsCount, homelessDeathsCount,
    -- partnershipsFormedCount, managedPopulationSummary, buildingSummary,
    -- tradeSummary, warnings.
    -- Rows with a null settlementId are silently skipped (defensive).
    -- The partial unique index settlement_turn_snapshots_transition_settlement_unique
