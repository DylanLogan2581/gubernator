-- Migration: validate_event_effects_same_world
-- Purpose: event_effects.resource_id, job_id, deposit_instance_id, and
-- managed_population_instance_id had no constraint or RPC check tying the
-- referenced row to the same world as the parent event. event_effects has no
-- world_id column (world scoping relies on the event join), and two of the
-- four referenced tables (deposit_instances, managed_population_instances)
-- only reach world_id via a settlement -> nation -> world join, so a
-- declarative composite FK is not possible for those two. Add RPC-side
-- validation (mirrors the join-based same-world triggers already used for
-- managed_population_instances and deposit_instance_resources) that runs for
-- every reference on every write path.
--
-- Changes:
--   1. New helper: public.validate_event_effect_world_membership(p_world_id, p_effect)
--      Raises a clear exception when a present reference does not belong to
--      p_world_id. Called from both write RPCs below, alongside the existing
--      validate_event_effect_fields check.
--   2. Re-creates create_event_group_with_events to call the new helper.
--   3. Re-creates update_event_group_with_events to call the new helper.
-- ============================================================================
-- Helper: per-effect same-world reference validation
-- ============================================================================
...
-- RPC: create_event_group_with_events (updated — adds same-world validation)
-- ============================================================================
...
-- RPC: update_event_group_with_events (updated — adds same-world validation)
-- ============================================================================
