-- Migration: cast_advance_world_turn_notification_type
-- Fixes a silent end_turn_transition_failed introduced by 20260531000002_notification_type_enum.
-- That migration promoted notifications.notification_type from text to the
-- public.notification_type enum, but advance_world_turn_if_current still
-- compared the column against a plain text local. Postgres has no implicit
-- text = enum operator, so the SELECT inside the duplicate-suppression
-- NOT EXISTS clause raised 42883 ("operator does not exist:
-- public.notification_type = text"). The RPC's `exception when others` block
-- swallowed the SQLSTATE and marked the transition `failed`, which the
-- end-turn-basic edge function surfaced to the client as a generic
-- end_turn_transition_failed 500.
--
-- Fix: cast v_notification_type to public.notification_type at the two sites
-- that mix it with the enum column (the INSERT projection and the duplicate
-- check). The cast also serves as input validation: an unknown enum label
-- raises 22P02 which the same handler converts into the same caller-facing
-- failure as before.
-- ---------------------------------------------------------------------------
...
begin
  ...
  where
    ...
    and w.status = 'active'
    and (
      w.owner_id = p_initiated_by_user_id
      or exists (
        select
          1
        from
      ...
      where
        not exists (
          ...
          where
            ...
            and n.notification_type = v_notification_type::public.notification_type
        );
    end if;
