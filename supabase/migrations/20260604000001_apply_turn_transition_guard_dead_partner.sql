-- Migration: apply_turn_transition_guard_dead_partner
-- Defense-in-depth guard for issue #499 / review finding H1.
--
-- The engine fix (phasePartnerships.ts) excludes citizens dying this turn from
-- partnership-formation eligibility. This migration adds a matching guard in the
-- §C32c partnership-changes section so that a 'formed' entry whose citizen_a or
-- citizen_b is already dead at apply time (e.g. they also appeared in citizenDeaths,
-- processed above) is rejected with P0001 rather than silently inserting a
-- partnership for a dead citizen.
-- ---------------------------------------------------------------------------
