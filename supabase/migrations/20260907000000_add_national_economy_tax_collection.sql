-- Migration: add_national_economy_tax_collection
-- #1083: national economy simulation phase — tax collection in kind. The
-- engine (phaseNationalEconomy) computes settlement->nation tax deltas and
-- per-nation turn summaries; this migration adds the persistence side:
-- nation_turn_snapshots (chart data, one row per nation per turn) and a new
-- apply_turn_transition helper that credits nation_resource_stockpiles
-- (added schema-only in 20260906000000) and writes the snapshot rows.
-- ---------------------------------------------------------------------------
-- nation_turn_snapshots
-- ---------------------------------------------------------------------------
create table public.nation_turn_snapshots (
  ...
  tax_collected_by_resource_json jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(),
  constraint nation_turn_snapshots_transition_world_fkey foreign key (turn_transition_id, world_id) references public.turn_transitions (id, world_id) on delete cascade,
  constraint nation_turn_snapshots_unique unique (turn_transition_id, nation_id)
);

create index nation_turn_snapshots_world_transition_idx on public.nation_turn_snapshots (world_id, turn_transition_id);

create index nation_turn_snapshots_nation_turn_idx on public.nation_turn_snapshots (nation_id, turn_number desc);

alter table public.nation_turn_snapshots enable row level security;

-- SELECT: any user with world access may read nation turn snapshots,
-- matching nation_resource_stockpiles visibility (v1: world members).
create policy "nation_turn_snapshots_select_world_access" on public.nation_turn_snapshots for
select
...
-- grants as the function owner), matching nation_resource_stockpiles' write
-- posture (20260906000000).
-- ---------------------------------------------------------------------------
-- internal_apply_turn_transition_nation_economy (§C37)
-- Credits nation_resource_stockpiles from the engine's nationStockpileDeltas
-- (always non-negative — tax collection only credits) and persists one
-- nation_turn_snapshots row per entry in nationTurnSnapshots.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Redefine apply_turn_transition with §C37 wired in. Body copied from the
-- latest definition (20260901000000) with: a v_valid_nation_ids cross-world
-- guard, guard loops for nationStockpileDeltas/nationTurnSnapshots, the §C37
-- call site, and its counts added to patchCounts.
-- ---------------------------------------------------------------------------
...
begin
  ...
  begin
    ...
    -- §C37: nation economy — credit nation_resource_stockpiles, persist
    -- nation_turn_snapshots (issue #1083).
    select
