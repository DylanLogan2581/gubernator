-- Migration: add_event_and_citizen_birth_death_notifications
-- Issue #883: Add notification producers for event.activated/event.expired and
-- citizen.born (aggregate)/citizen.died (per-citizen).
--
-- Changes:
--   1. Expand the dedup index to include event_id and citizen_id so that
--      multiple per-event or per-citizen rows for the same recipient in the
--      same transition are allowed (and retries are still deduplicated).
--   2. Add FK constraints on notifications.event_id → events and
--      notifications.citizen_id → citizens, replacing the old placeholder
--      comments with real referential integrity.
--   3. Extend internal_apply_turn_transition_log_entries_and_notifications with
--      four new generator blocks:
--        · event.activated   (per event, from eventStatusPatches payload; fires
--                              when fromStatus = 'pending'; scope-aware recipients)
--        · event.expired     (per event, from eventStatusPatches payload; fires
--                              when toStatus = 'expired'; scope-aware recipients)
--        · citizen.born      (aggregate per settlement, from citizen.born log
--                              entries; citizen_id not set — birth IDs are not
--                              known at log-emission time)
--        · citizen.died      (per citizen, from citizenDeaths payload;
--                              citizen_id set; inner join citizens for settlement)
--
-- Recipient scoping:
--   event settlement-scoped: settlement manager + nation manager + world admins + super admins
--   event nation-scoped:     nation manager + world admins + super admins
--   event world-scoped:      world admins + super admins
--   citizen.born/died:       settlement manager + nation manager + world admins + super admins
-- ---------------------------------------------------------------------------
-- ---------------------------------------------------------------------------
-- 1. Expand the dedup index to include event_id and citizen_id columns.
--
--    The old index keyed on (transition, recipient, type, coalesce(settlement_id, sentinel)).
--    Multiple events or citizens in the same transition would collide when they
--    share a scope (e.g. two world-scoped events both produce settlement_id=null).
--
--    The new index adds coalesce(event_id, sentinel) and coalesce(citizen_id, sentinel)
--    before settlement_id so distinct entities are never merged.
--
--    Existing rows have event_id = null and citizen_id = null; their effective key
--    changes from (…, coalesce(settlement_id, sentinel)) to
--    (…, sentinel_event, sentinel_citizen, coalesce(settlement_id, sentinel)) — a
--    strict superset, so no existing conflict can arise and idempotency is preserved.
-- ---------------------------------------------------------------------------
drop index if exists public.notifications_transition_dedup_idx;

create unique index notifications_transition_dedup_idx on public.notifications (
...
where
  generated_in_transition_id is not null;

-- ---------------------------------------------------------------------------
-- 2. Column comments for event_id and citizen_id (FK constraints already exist
--    from 20260624000000_add_placeholder_fk_constraints.sql).
-- ---------------------------------------------------------------------------
comment on column public.notifications.event_id is 'FK to events; set for event.activated and event.expired notifications.';

comment on column public.notifications.citizen_id is 'FK to citizens; set for citizen.died (per-citizen) notifications.
   citizen.born is aggregate (settlement-scoped) — citizen_id is null.';

-- ---------------------------------------------------------------------------
-- 3. Extend internal_apply_turn_transition_log_entries_and_notifications.
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- §C33g: citizen.born — aggregate per settlement (one notification per settlement,
  -- not per individual citizen).  citizen_id is not set because the newborn
  -- citizen's database ID is not known at log-emission time (the log entry is
  -- emitted by the simulation engine before the INSERT assigns a UUID).
  -- Reads from the citizen.born log entries written in §C33a above.
  ...
  select
    ...
    'A citizen was born in this settlement.',
    p_transition_id
  from (
    select distinct settlement_id
    from public.turn_log_entries
