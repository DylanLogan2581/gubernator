-- Migration: drop_apply_turn_transition_4param_overload
--
-- The consolidation migration (20260619000001) re-created the original 4-param
-- apply_turn_transition signature AFTER the forecast-snapshot migration
-- (20260611000001) added a 5-param overload with p_forecast_snapshot_jsonb
-- DEFAULT NULL. Because CREATE OR REPLACE with a different signature creates a
-- NEW overload instead of replacing the old one, both overloads now coexist in
-- the catalog. Any call with 4 positional args is ambiguous (42725).
--
-- Additionally the stale 5-param overload from 20260611000001 calls internal
-- helper functions with signatures that were changed by the subsequent refactor
-- (20260604000007), so it is non-functional.
--
-- Fix: drop both overloads, then recreate as a single 5-param function
-- (backward-compatible via DEFAULT NULL) whose body is taken from the
-- consolidation migration and extended to persist p_forecast_snapshot_jsonb into
-- turn_transitions.forecast_snapshot_jsonb.
-- ---------------------------------------------------------------------------
-- Drop both existing overloads
drop function if exists public.apply_turn_transition (uuid, integer, jsonb, uuid);

drop function if exists public.apply_turn_transition (uuid, integer, jsonb, uuid, jsonb);

-- ---------------------------------------------------------------------------
-- Unified apply_turn_transition (5-param, 5th defaults to NULL)
-- ---------------------------------------------------------------------------
create function public.apply_turn_transition (
  p_world_id uuid,
