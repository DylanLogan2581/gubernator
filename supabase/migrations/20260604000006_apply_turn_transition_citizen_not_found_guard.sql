-- Migration: apply_turn_transition_citizen_not_found_guard
-- Closes review finding M18 (issue #525).
--
-- Replaces three silent fall-through cases in apply_turn_transition where
-- `SELECT INTO <function-scope var>` left the variable unchanged when no row
-- matched, making the subsequent value-test run against a stale value:
--
--   §C32b: v_citizen_type (PC-immunity check) — if citizen not found, raise
--          instead of silently succeeding against the previous iteration's type.
--
--   §C32c active branch: v_citizen_status (dead-partner check) — if citizen_a
--          or citizen_b not found, raise instead of testing a stale status.
--
--   §C32c else branch: v_partnership_id (widowing/dissolution) — if no active
--          partnership exists between the two citizens, raise instead of
--          silently updating the wrong partnership (or doing nothing).
-- ---------------------------------------------------------------------------
...
begin
  ...
  begin
    ...
    -- values from carrying over when a row is unexpectedly absent.
    for v_partnership_change in
      select value
      from jsonb_array_elements(coalesce(p_payload -> 'partnershipChanges', '[]'::jsonb))
    loop
