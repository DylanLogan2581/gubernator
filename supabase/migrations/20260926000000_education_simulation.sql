-- Migration: education_simulation
-- #1104: the end-turn simulation gains an education phase (enrollment
-- progress, teacher staffing, graduation). This migration wires the DB write
-- path: a settlement_turn_snapshots column for the per-settlement education
-- summary, a new internal helper that applies citizen education-level
-- patches / enrollment progress updates / enrollment graduations, and the
-- corresponding extension of apply_turn_transition's payload handling.
-- ---------------------------------------------------------------------------
-- §1: settlement_turn_snapshots gains education_summary_json (nullable
-- jsonb passthrough, mirrors managed_populations_summary_json etc). The
-- production write path goes through the SECURITY DEFINER RPC below, which
-- bypasses column grants entirely (runs as function owner) -- the grant is
-- added anyway for consistency with every other snapshot column being
-- explicitly granted (defense-in-depth for any hypothetical direct-insert
-- caller).
-- ---------------------------------------------------------------------------
alter table public.settlement_turn_snapshots
add column education_summary_json jsonb;

grant insert (education_summary_json) on public.settlement_turn_snapshots to authenticated;

-- ---------------------------------------------------------------------------
-- §2: internal_apply_turn_transition_education_patches -- applies
-- citizenEducationPatches / enrollmentProgressUpdates / enrollmentGraduations
-- from the simulation payload. Modeled on
-- internal_apply_turn_transition_citizen_partnership_patches's loop style.
-- ---------------------------------------------------------------------------
create or replace function public.internal_apply_turn_transition_education_patches (
  p_payload jsonb,
  out citizen_education_patch_count integer,
  out enrollment_progress_update_count integer,
  out enrollment_graduation_count integer
...
begin
  citizen_education_patch_count := 0;
  enrollment_progress_update_count := 0;
  enrollment_graduation_count := 0;
  ...
  loop
    ...
    set
      education_level_id = (v_citizen_education_patch ->> 'educationLevelId')::uuid
    where
      id = (v_citizen_education_patch ->> 'citizenId')::uuid;

    citizen_education_patch_count := citizen_education_patch_count + 1;
  end loop;
  ...
  loop
    ...
    set
      progress_turns = (v_enrollment_progress_update ->> 'progressTurns')::integer,
      target_level_id = (v_enrollment_progress_update ->> 'targetLevelId')::uuid
    where
      id = (v_enrollment_progress_update ->> 'enrollmentId')::uuid;

    enrollment_progress_update_count := enrollment_progress_update_count + 1;
  end loop;
  ...
  loop
    ...
    where id = (v_enrollment_graduation ->> 'enrollmentId')::uuid;

    enrollment_graduation_count := enrollment_graduation_count + 1;
  end loop;
end;
$$;

revoke all on function public.internal_apply_turn_transition_education_patches (jsonb)
from
  public,
  anon,
  authenticated;
...
-- ---------------------------------------------------------------------------
-- §3: internal_apply_turn_transition_settlement_snapshots -- full
-- redefinition (copied verbatim from 20260619000001) with
-- education_summary_json added to the insert column list and
-- snap.value -> 'educationSummary' added to the select list.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- §4: apply_turn_transition -- full redefinition (copied verbatim from
-- 20260918000001_currency_confidence_snapshots.sql, the truly-current
-- definition as of this migration) with:
--   - v_valid_education_level_ids / v_valid_enrollment_ids id-sets
--   - cross-world guards for citizenEducationPatches, enrollmentProgressUpdates,
--     enrollmentGraduations
--   - a call to internal_apply_turn_transition_education_patches
