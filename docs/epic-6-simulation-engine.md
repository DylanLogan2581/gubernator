...
| 10  | `homelessness`       | Citizens over the population cap die of homelessness. The engine plays no role in the deconstruct-overshoot ledger; §C32e in `apply_turn_transition` stamps those log entries with `turn_transition_id` for auditing, and the population cap is already correctly lower from the deconstructed building being absent from the active-buildings list. |
